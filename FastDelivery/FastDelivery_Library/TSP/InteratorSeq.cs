...
namespace FastDelivery_Library
{
    ...
    private int nbCandidats;
        ...
        public IteratorSeq(ICollection<int> nonVus, int sommetCrt, int[,] cout, int[] duree)
        {
            ...
            foreach (int s in nonVus)
            {
                int indexMin = listeCouts.IndexOf(listeCouts.Min());
                candidats[nbCandidats++] = nonVus.ElementAt(indexMin);
                listeCouts[indexMin] = int.MaxValue;
            }
            candidats = candidats.Reverse().ToArray();
        }
